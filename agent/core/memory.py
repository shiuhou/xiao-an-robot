...
class XiaoAnMemoryStore:
    ...
    def insert_work_activity(
        ...
        active_event_text = (
            ...
            else f"{app_name} | {activity_type} | {project_hint}"
        )
    ...
    def query_recent_work_activities(
        ...
        if keyword:
            clauses.append(
                "(app_name LIKE ? OR window_title LIKE ? OR "
                "activity_type LIKE ? OR project_hint LIKE ?)"
            )
            pattern = f"%{keyword}%"
    ...
    def insert_summary(
        ...
        timestamp_ms: int | None = None,
        privacy_level: str = "normal",
        event_type: str = "summary.generated",
        event_payload: dict[str, Any] | None = None,
        session_id: str | None = None,
    ) -> dict[str, int]:
