"""Continuous fixed-window ASR demo built on the robot /audio PCM stream."""
...
async def listen_and_transcribe(args: argparse.Namespace) -> int:
    ...
    while args.max_seconds <= 0 or time.monotonic() - start < args.max_seconds:
        ...
        if event is not None:
            ...
            wav_path = output_dir / f"continuous_utterance_{stamp}_{utterances}.wav"
            trim_path = output_dir / f"continuous_utterance_{stamp}_{utterances}.trim.wav"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Continuously detect one utterance from latest_audio.pcm and run ASR.")
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        print(f"continuous_asr_demo failed: {exc}", file=sys.stderr)
        return 2
