...
4. base-station mic / mock text → ASR/context → OpenClaw Gateway → 主动关怀 Demo — **mock transcript + real OpenClaw route P；real mic 已触发 care，动作/TTS 顺序已改为 move-out -> left-turn -> expression -> streamed TTS**
...
| **Visual console mainline trace 2026-07-06** | `base_station/monitor/emotion_runtime.py`, `base_station/integration_console/visual_trace.py`, `docs/runbooks/integration_console.md` | 正式视觉入口新增 `--source ws_video`：机器人 `/video` 帧经 `ws_server` 注入主链路 frame source，OpenFace/Gate/VLM 已计算结果通过 `VisualTracePublisher` observer 写给 Integration Console；console 只是旁路展示，不替代主线 `VLM -> Brain/OpenClaw -> /agent -> /control`。Targeted unit tests PASS；真实机器人 `/video` 待现场验收。 |
| **Demo robot burn 2026-07-04** | `robot/mergetesting/src/config.local.h` ignored local config, `mergetesting_care_demo_face240_spoken_tts_din41` | Linux `/dev/ttyACM0` USB upload PASS after temporary port permission fix. Local config points robot to SSID `PnX` and base station `192.168.31.252:8765` without committing secrets. Serial boot verified face240/motor enabled, video/audio disabled, robot WiFi IP `192.168.31.176`, and `/control` retrying until base-station WS server starts. |
| **DIN41 OTA demo env 2026-07-05** | `robot/mergetesting/platformio.ini` | Added `mergetesting_care_demo_face240_spoken_tts_din41_ota_usb` for USB first flash and `mergetesting_care_demo_face240_spoken_tts_din41_ota` for subsequent wireless updates. USB flash to `/dev/ttyACM0` PASS; serial showed `OTA Ready hostname=xiao-an-esp32 auth=disabled`; wireless `espota` upload to `192.168.31.176` from host `192.168.31.252` PASS with `Result: OK`. |
| **Care demo 串行动作/TTS 2026-07-05** | `agent/skills/robot_motion.py`, `agent/core/action_executor.py`, `agent/core/gateway.py`, `docs/status/2026-07-05.md` | `xiaoan.robot.care` 不再先播 `reply_text` 或使用 local `care_01`；现在顺序为 `motion.execute move_out_of_dock` -> `motion.execute turn` (left, `angle_deg=-25`) -> `display.expression caring` -> `audio.play_tts` streamed TTS。现场 mock text “小安我累了” 经真实 OpenClaw 返回 care；server log 看到 move-out/turn/expression/TTS ack 与 `audio.playback_done ok`。TTS ack timeout 已从 gateway 侧修正，目标测试 PASS 66 tests。剩余：用户仍听到轻微爆音，且 move-out 体感不足；当前 care 仍是 `speed=0.56` + `distance_cm=10` + `timeout_ms=1200`，下一步改为显式 `duration_ms=2000`。 |
| **Demo 1 mic/ASR gain preflight 2026-07-05** | `tools/demo/demo1_usb_mic_to_agent_screen.py`, `base_station/perception/audio_segments.py`, `base_station/monitor/asr_runtime.py`, `docs/runbooks/demo1_usb_mic_to_agent_screen.md` | Demo 1 mic path now fixes ASR WAV input to `16 kHz / mono / pcm_s16le`, records `details.input_level.max_volume_dbfs/gain_status/clipping_percent`, and starts speech trimming with `min_speech_ms=350`, `pre_roll_ms=250`, `end_silence_ms=800`, noise reduction off. Target tests PASS 60; mock text “小安我累了” reached real OpenClaw decision-only and returned validated `xiaoan.robot.care`. Linux session did not show the USB mic, so real mic retest remains现场 step. |
| **Care demo move-out 2200ms 2026-07-05** | `agent/skills/robot_motion.py`, `agent/core/action_executor.py`, `base_station/ws_server/server.py` | Care move-out now defaults to `speed=0.56`, `duration_ms=2200`, `timeout_ms=2600`; post-move delay raised to 2.35s so left turn no longer interrupts move-out. Target tests PASS 73. Real robot mock text “小安我累了” completed move_out success at ~2.2s, then turn/expression/TTS; server log showed `Motion completed ... success` before turn and `audio.playback_done ok`. USB mic is visible as `UACDemoV1.0`; gain still needs final calibration. |
| **Demo 1 SenseVoice local loader fix 2026-07-05** | `base_station/perception/asr.py`, `tests/unit/test_asr.py` | SenseVoice local ASR no longer passes `trust_remote_code=True` for the populated local model path, avoiding the misleading FunASR remote-code install/import path (`No module named 'model'`). Interrupted live run actually completed: ASR recognized “小安我有点累了”, OpenClaw returned `xiaoan.robot.care`, and `/agent -> /control` produced move-out/turn/expression/TTS acks plus `audio.playback_done ok`. Verification: `.venv/bin/python -m unittest tests.unit.test_asr tests.unit.test_asr_runtime tests.unit.test_audio_segments tests.unit.test_demo1_usb_mic_to_agent_screen` PASS 80; replaying `runtime/demo1_audio/demo1_usb_mic_20260705_172922.asr.wav` returned “小安我有点累了”. |
| **Demo 1 speaker bench restore 2026-07-05** | `docs/status/2026-07-05.md`, `robot/mergetesting/platformio.ini` | Speaker probe on DIN41 confirmed tones audible at amplitudes `2000/8000/16000/28000`, with distortion only at `28000`; embedded phrase became audible at gain64; restored demo firmware `mergetesting_care_demo_face240_spoken_tts_din41_ota_usb` plus base-station `XIAOAN_TTS_TARGET_PEAK=800`. Direct Chinese TTS and full Demo 1 mock path both completed with `audio.playback_done ok`; user confirmed audible but too quiet, so next pass should raise spoken volume conservatively below the distortion boundary. |
| **New speaker TTS baseline 2026-07-08** | `robot/mergetesting/platformio.ini`, `robot/mergetesting/src/speaker.cpp`, `docs/runbooks/spoken_tts_demo_smoke.md`, `docs/agents/05_test_matrix.md` | Replaced speaker is `4 ohm 3 W, 500-5000 Hz`; accepted path is `mergetesting_care_demo_face240_spoken_tts_din41_ota_usb`, MAX98357A BCLK=39/LRC=40/DIN=41, `MERGETEST_SPEAKER_BUFFERED_STREAM=1`, `MERGETEST_SPEAKER_STREAM_GAIN=32`, base `XIAOAN_TTS_TARGET_PEAK=500`. Firmware buffers the full PCM stream and only starts I2S playback after `audio.stream_end`. On Windows SAPI, `XIAOAN_TTS_VOICE='Microsoft Hanhan Desktop'` was the validated fallback; on Linux the default helper uses `XIAOAN_EDGE_TTS_VOICE` instead. Evidence: `audio.playback_done status=ok bytes_written=367384 duration_ms=5763`; user feedback: very clear. |
| **Buffered TTS serial markers 2026-07-08** | `robot/mergetesting/src/speaker.cpp` | Added explicit firmware serial markers for future TTS checks: `buffered pcm playback start bytes=...` at playback task start and `buffered pcm task done ok=... bytes_written=...` at completion. Existing `pcm buffered playback done ...` log remains. Target tests PASS 55; `mergetesting_care_demo_face240_spoken_tts_din41_ota_usb` build SUCCESS. |
| **Buffered TTS OTA check 2026-07-08** | `robot/mergetesting` OTA, `runtime/ws_server.log`, `runtime/ws_state.json` | OTA upload of marker rebuild to robot `192.168.31.175` succeeded via `espota` host `192.168.31.253:43564` after stale host IP `192.168.31.252` failed. Robot reconnected with reset reason `software`. Exactly one console-path TTS (`/api/robot/tts`, text `小安，這是 buffered firmware check。`) was sent; robot returned `command.ack audio.play_tts accepted` and `audio.playback_done status=ok bytes_written=184468 duration_ms=2917`. No `/dev/ttyACM*` or `/dev/ttyUSB*` was visible in this environment, so serial-only marker lines were not captured. |
| **Buffered TTS direct serial check 2026-07-08** | `/dev/ttyACM0`, `runtime/ws_server.log`, `runtime/tts/tts-6b5cbeab-1783526295929.wav` | Direct `/agent` TTS text `你好，我是小安。請聽我的中文發音清不清楚。` reached robot and serial showed `pcm stream begin sample_rate=16000 channels=1 drain_only=0 buffered=1`, but no `buffered pcm playback start` / `buffered pcm task done` and no old `pcm stream task done`; `/control` reconnected with `reset_reason=poweron` before playback_done. CLI returned `agent.ack ok=false error=Robot connection is closed: xiaoan_robot_01`. WAV was valid 16 kHz mono pcm_s16le; `readframes` PCM bytes 158976, normalized peak 500. Buffered runtime playback is not confirmed by this run. |
| **Buffered TTS receive diagnostics 2026-07-09** | `robot/mergetesting/src/speaker.cpp`, `/dev/ttyACM0`, `runtime/ws_server.log` | Added temporary receive-side logs: begin free heap/PSRAM, 32 KiB buffered append progress, stream_end buffered bytes/failed flag. USB uploaded `mergetesting_care_demo_face240_spoken_tts_din41_ota_usb` to `/dev/ttyACM0`; build/upload PASS. Three direct `/agent` TTS runs all succeeded without tuning TTS parameters: `你好` buffered 38400 bytes -> `audio.playback_done ok bytes_written=65992 duration_ms=1060`; `你好，我是小安。` buffered 66816 bytes -> `bytes_written=122828 duration_ms=1940`; long text `你好，我是小安。請聽我的中文發音清不清楚。` buffered 158976 bytes -> `bytes_written=307148 duration_ms=4820`. Serial showed `stream_end`, `buffered pcm playback start`, and `buffered pcm task done` for all three; no reset/断链 reproduced. Runtime PSRAM was visible (`free_psram=8370979`) despite PlatformIO board summary saying No PSRAM. |
...
- Added `docs/runbooks/openclaw_preflight_acceptance.md` as the current
  ...
  The 2026-07-08 baseline supersedes the earlier low-volume `800` setting:
  use base station `XIAOAN_TTS_TARGET_PEAK=500`, ESP stream gain `32`, and
  buffered PCM playback on the DIN41 firmware path. `XIAOAN_TTS_VOICE` only
  applies to Windows SAPI; Linux edge-tts uses `XIAOAN_EDGE_TTS_VOICE`.
