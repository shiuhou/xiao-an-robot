...
| `handle_audio` | /audio | 收机器人 PCM，更新 `runtime/latest_audio.pcm` / audio stats；作为 robot-mic fallback/diagnostics；正式 9 天 demo 主输入改为 DK-2500/base-station mic |
...
| base-station mic capture target | ⬜ | 9 天 demo 主线：DK-2500 mic -> WAV/audio_file -> ASR -> context -> OpenClaw/Agent -> `/control` |
| `audio_diagnostics.py` | ✅ | robot `/audio` WAV/PCM diagnostics, RMS/peak/DC/clipping checks before ASR tuning |
...
| `fixed_window_asr_demo.py` | 🟡 | Rolling `runtime/latest_audio.pcm` robot `/audio` fallback demo; `continuous_asr_demo.py` remains a compatibility wrapper |
