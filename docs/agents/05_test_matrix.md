...
| Base-station mic demo | `tools/demo/demo1_usb_mic_to_agent_screen.py`, `tests/unit/test_demo1_usb_mic_to_agent_screen.py` | DK-2500 USB mic -> WAV/audio_file -> ASR transcript -> fixed `demo1.openclaw_context.v1` context -> OpenClaw Gateway `xiaoan-runtime` -> validated tool_calls -> ActionExecutor -> `/agent`; mock fallback writes `source=mock` | P/H partial: 2026-07-03 `tests.unit.test_demo1_usb_mic_to_agent_screen`; mock `--route-openclaw` with “小安，我有点累” produced `openclaw_send.ok=true`, `xiaoan.robot.care`, and `/agent` ack ok for expression/motion/TTS; expression subpath also has robot USB serial `command.ack display.expression -> ok`; real mic OpenClaw route and motion `motion.completed` pending |
...
| 4 | 基站 mic 关怀闭环 | DK-2500 mic -> ASR -> OpenClaw Gateway / `xiaoan-runtime` -> ActionExecutor -> `/agent` -> `/control` -> ack/completed | P/H partial: 2026-07-03 mock text completed real OpenClaw Gateway decision path and `/agent` acks; expression subpath verified robot-side `command.ack ok`; full real-mic pass plus robot-side `command.ack` for care sequence and matching `motion.completed` still need dedicated hardware pass |
