...
| `mergetesting_care_demo_face240_spoken_tts_din41_ota_usb` | P/H: 2026-07-05 USB upload PASS on `/dev/ttyACM0`; enables ArduinoOTA for the DIN41 spoken TTS demo path | H: serial boot showed WiFi `PnX`, robot IP `192.168.31.176`, `OTA Ready hostname=xiao-an-esp32 auth=disabled`, face240/motor/speaker enabled, robot mic/video disabled. Late speaker restore pass: after USB reset robot reconnected as `192.168.31.175`; with base-station `XIAOAN_TTS_TARGET_PEAK=800`, direct Chinese TTS completed `audio.playback_done ok bytes=66816 duration_ms=2999`, and full Demo 1 mock completed `audio.playback_done ok bytes=159744 duration_ms=5519`. User heard speech but volume remains small. |
| `mergetesting_care_demo_face240_spoken_tts_din41_ota` | P/H: 2026-07-05 wireless `espota` upload PASS to `192.168.31.176` from host `192.168.31.252` | H: PlatformIO OTA reported `Result: OK` and `Success`; use this env for future no-COM updates while the robot remains on `PnX` |
