...
| Base-station mic demo | `tools/demo/demo1_usb_mic_to_agent_screen.py`, `tests/unit/test_demo1_usb_mic_to_agent_screen.py` | DK-2500 USB mic -> WAV/audio_file -> ASR transcript -> fixed `demo1.openclaw_context.v1` context -> filtered Demo 1 OpenClaw tool manifest -> OpenClaw Gateway `xiaoan-runtime` -> strict validated tool_calls -> ActionExecutor -> `/agent`; mock fallback writes `source=mock` | P/H partial: 2026-07-03 `tests.unit.test_demo1_usb_mic_to_agent_screen`; mock transcript with real `--route-openclaw` route and “小安，我有点累” produced `openclaw_send.ok=true`, `xiaoan.robot.care`, and `/agent` ack ok for expression/motion/local `care_01`; robot log showed expression/motion `command.ack ok` and `motion.completed success`, but local audio returned `AUDIO_UNSUPPORTED speaker not ready`; real mic ASR target phrase still pending |
...
| Integration Console | `tests/unit/test_integration_console_server.py`, `tests/unit/test_ws_state_snapshot.py` | `/api/health`, `/api/state` without runtime files, `/api/latest-image` missing-file behavior, robot command payload construction, scenario step ordering, tool whitelist rejection, `ws_state.json` reader and atomic writer | P: 2026-07-04 bundled Python `-m unittest tests.unit.test_integration_console_server tests.unit.test_ws_state_snapshot` -> OK 9 tests, 2 skipped because this local Python lacks `websockets`; full discover on same env currently FAILS with existing missing optional deps (`websockets`, `cv2`, `matplotlib`) and legacy emotion-care expectation failures |
...
| 4 | 基站 mic 关怀闭环 | DK-2500 mic -> ASR -> OpenClaw Gateway / `xiaoan-runtime` -> ActionExecutor -> `/agent` -> `/control` -> ack/completed | P/H partial: 2026-07-03 mock text completed real OpenClaw Gateway decision path and `/agent` acks; robot log showed command ack for expression/motion and matching `motion.completed`; local audio ack failed with `speaker not ready`; full real-mic pass still needs stable target phrase ASR |
