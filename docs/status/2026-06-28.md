...
This file records the 2026-06-28 DK2500/OpenClaw integration status after the 2026-06-27 full hardware handoff. It should be read together with `docs/status/2026-06-27.md`.
...
## 2026-06-28 Speaker OTA/TTS Investigation

`mergetesting_speaker_only_ota` was recovered and OTA-uploaded over the Windows hotspot with ESP32 IP `192.168.137.147` and host IP `192.168.137.1`. `audio.play_local care_01` returned `command.ack` ok over `/control`.

The experimental base-station spoken TTS path now synthesizes PCM and can forward it over `/control`, but live hardware playback is not stable yet. The ESP32 reports `reset=interrupt_wdt` after streamed/buffered PCM playback attempts. A drain-only diagnostic env that accepts PCM without I2S playback avoids the reset, so the remaining failure is in the real PCM-to-I2S playback path, not the OTA/IP setup. Next debug step requires USB serial panic/backtrace capture, because network logs only expose `reset_reason`.

Follow-up validation on the same day captured the serial/network symptom and made the unsafe PCM spoken-TTS route opt-in only. Default `audio.play_tts` now forwards metadata only and lets firmware use the existing local mock-tone path; set `XIAOAN_CONTROL_TTS_STREAM=1` only for PCM-stream diagnostics. With the restarted base-station server and robot at `192.168.137.147`, `audio.play_tts` and `audio.play_local care_01` both returned `command.ack` ok. One delayed `interrupt_wdt` reconnect was observed after an earlier mixed TTS/local-audio sequence, so isolated retests were run: default `audio.play_tts` had no new WDT reconnect for 60 seconds, and `audio.play_local care_01` also had no new WDT reconnect for 60 seconds. COM19 is no longer required for discovering the OTA IP; the robot reports it in `device.hello`, and OTA uses it as `--upload-port` while the Windows hotspot host remains `192.168.137.1`.

Late live follow-up restored the hotspot and OTA path with robot IP `192.168.137.93` and host IP `192.168.137.1`. Multiple `mergetesting_speaker_only_ota` builds and OTA uploads succeeded while the base station was running with `XIAOAN_CONTROL_TTS_STREAM=1`. PCM playback strategies tested on hardware included buffered playback, direct chunk playback, queue-driven chunk playback, and finally queue-driven playback using the same bounded I2S write helper as local tones. All variants still reset with `reset=interrupt_wdt` shortly after `audio.play_tts` ack, including the short sentence `Hi.`. The drain-only env remains the key control: PCM receive/forwarding works when I2S playback is skipped, so the remaining fault is inside ESP32 PCM-to-I2S playback, the I2S driver/interrupt layer, pins, or power. Next required step is USB serial panic/backtrace capture; network logs only show the reset reason.

An additional no-stream diagnostic generated an embedded full sentence, `I can speak now.`, into `mergetesting_speaker_phrase_only_ota` so `/control audio.play_tts` could trigger PCM playback without WebSocket binary audio. That also reset with `reset=interrupt_wdt` and the user reported no audible sentence. The robot was then restored to the safe `mergetesting_speaker_only_ota` firmware; `audio.play_local care_01` returned ack and no new WDT appeared during the follow-up observation window.
