...
void setup() {
    ...
    waitForSerialWindow();
    Serial.println();
    Serial.println("[FACE240] RoboEyes hybrid: pose morph + flat primitives");
    Serial.println("[FACE240] push=full screen first; auto margin if fps<26");
    Serial.println("[FACE240] Serial: 1-9 face, m auto, f toggle full/margin push");
    Serial.println("[FACE240] Pins: SCLK=12 MOSI=11 CS=10 DC=9 RST=14 BL=21");
    Serial.printf("[FACE240] framebuffer=%u bytes spi=%luHz\n",
