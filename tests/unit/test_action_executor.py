...
class FakeMemoryStore:
    ...
    def insert_work_activity(self, **kwargs) -> dict:
        ...
        return {
            "event_id": len(self.work_activities),
            "work_activity_id": len(self.work_activities),
        }
...
class ActionExecutorTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_unknown_tool_records_failed(self) -> None:
        memory_store = FakeMemoryStore()
        executor = ActionExecutor(FakeRobotMotionSkill(), memory_store=memory_store)
        decision = OpenClawDecision(
            handled=True,
    ...
    async def test_local_tool_ok_false_records_failed(self) -> None:
        memory_store = FakeMemoryStore()
        local_tools = FakeLocalToolRegistry(result={
            "ok": False,
            "name": "note.add",
    ...
    async def test_work_context_record_persists_activity_and_records_tool_run(self) -> None:
        memory_store = FakeMemoryStore()
        executor = ActionExecutor(FakeRobotMotionSkill(), memory_store=memory_store)
        decision = OpenClawDecision(
            handled=True,
        ...
        self.assertEqual(memory_store.work_activities[0]["window_title"], "coding")
    ...
    async def test_reminder_validation_failures_record_failed_tool_runs(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            db_path = str(Path(temp_dir) / "reminder_validation_failures.db")
            with XiaoAnMemoryStore(db_path) as memory_store:
                executor = ActionExecutor(
                    FakeRobotMotionSkill(),
                    memory_store=memory_store,
                )

                missing_message = await executor.execute(OpenClawDecision(
                    handled=True,
                ...
                missing_time = await executor.execute(OpenClawDecision(
                    handled=True,
                    tool_calls=[OpenClawToolCall(
                        name="reminder.add",
                        arguments={"message": "wake me"},
                    )],
                ))
                not_found = await executor.execute(OpenClawDecision(
                    handled=True,
                    tool_calls=[OpenClawToolCall(
                        name="reminder.cancel",
                ...
                self.assertEqual(
                    missing_message["skipped_actions"][0]["reason"],
                    "local_tool_failed",
                )
                self.assertEqual(
                    missing_time["skipped_actions"][0]["result"]["error"],
                    "missing_reminder_time",
                )
                self.assertTrue(
                    missing_time["skipped_actions"][0]["result"][
                        "needs_time_clarification"
                    ],
                )
                self.assertEqual(
                    not_found["skipped_actions"][0]["result"]["error"],
                    "reminder_not_found",
                )
                runs = memory_store.query_recent_tool_runs(limit=10)
                errors = [run["error"] for run in runs]
                self.assertIn("missing_reminder_message", errors)
                self.assertIn("missing_reminder_time", errors)
                self.assertIn("reminder_not_found", errors)
                self.assertTrue(all(run["status"] == "failed" for run in runs))
                self.assertEqual(memory_store.query_reminders(include_fired=True), [])
    ...
    async def test_task_validation_failures_record_failed_tool_runs(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            db_path = str(Path(temp_dir) / "task_validation_failures.db")
            with XiaoAnMemoryStore(db_path) as memory_store:
                executor = ActionExecutor(
                    FakeRobotMotionSkill(),
                    memory_store=memory_store,
                ...
                missing_title = await executor.execute(OpenClawDecision(
                    handled=True,
                    tool_calls=[OpenClawToolCall(name="task.add", arguments={})],
                ))
                not_found = await executor.execute(OpenClawDecision(
                    handled=True,
                    tool_calls=[OpenClawToolCall(
                        name="task.complete",
                        arguments={"title_contains": "missing"},
                    )],
                ))
                cancel_not_found = await executor.execute(OpenClawDecision(
                    handled=True,
                    tool_calls=[OpenClawToolCall(
                        name="task.cancel",
                ...
                self.assertEqual(
                    missing_title["skipped_actions"][0]["reason"],
                    "local_tool_failed",
                )
                self.assertEqual(
                    not_found["skipped_actions"][0]["reason"],
                    "local_tool_failed",
                )
                self.assertEqual(
                    cancel_not_found["skipped_actions"][0]["reason"],
                    "local_tool_failed",
                )
                runs = memory_store.query_recent_tool_runs(limit=10)
                errors = {run["tool_name"]: run["error"] for run in runs}
                self.assertEqual(errors["task.add"], "missing_task_title")
                self.assertEqual(errors["task.complete"], "task_not_found")
                self.assertEqual(errors["task.cancel"], "task_not_found")
                self.assertTrue(all(run["status"] == "failed" for run in runs))

    async def test_project_tools_persist_business_events_and_tool_runs(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            db_path = str(Path(temp_dir) / "action_executor_project_memory.db")
            with XiaoAnMemoryStore(db_path) as memory_store:
                executor = ActionExecutor(FakeRobotMotionSkill(), memory_store=memory_store)

                await executor.execute(OpenClawDecision(
                    handled=True,
                    tool_calls=[OpenClawToolCall(
                        name="note.add",
                        arguments={"content": "project note"},
                    )],
                ...
                await executor.execute(OpenClawDecision(
                    ...
                    tool_calls=[OpenClawToolCall(
                        name="work_context.record",
                        arguments={"content": "editing memory", "app_name": "VS Code"},
                    )],
                ), source_event_type="frontend.message")
                await executor.execute(OpenClawDecision(
                    handled=True,
                ...
                self.assertEqual(memory_store.query_recent_notes()[0]["content"], "project note")
                self.assertEqual(
                    memory_store.query_recent_work_activities()[0]["window_title"],
                    "editing memory",
                )
                self.assertEqual(
                    memory_store.query_recent_summaries()[0]["summary_type"],
                    "daily",
                )
                event_types = [
                    event["event_type"]
                ...
                self.assertIn("work_context.record", event_types)
                self.assertIn("summary.daily", event_types)
                self.assertEqual(event_types.count("tool.run"), 3)
                runs = memory_store.query_recent_tool_runs(limit=10)
                self.assertEqual({run["tool_name"] for run in runs}, {
                    "note.add",
                    "work_context.record",
                    "summary.daily",
                })
                ...
                self.assertTrue(all(
                    run["source_event_type"] == "frontend.message"
                    for run in runs
                ))

    async def test_unknown_and_missing_argument_tools_record_failed_runs(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            db_path = str(Path(temp_dir) / "action_executor_failed_tools.db")
            with XiaoAnMemoryStore(db_path) as memory_store:
                executor = ActionExecutor(FakeRobotMotionSkill(), memory_store=memory_store)

                unknown_result = await executor.execute(OpenClawDecision(
                    handled=True,
                    tool_calls=[OpenClawToolCall(name="unknown.tool")],
                ))
                missing_result = await executor.execute(OpenClawDecision(
                    handled=True,
                    tool_calls=[OpenClawToolCall(name="note.add", arguments={})],
                ))

                self.assertEqual(unknown_result["skipped_actions"][0]["reason"], "unknown_tool")
                self.assertEqual(
                    missing_result["skipped_actions"][0]["reason"],
                    "local_tool_failed",
                )
                runs = memory_store.query_recent_tool_runs(limit=10)
                ...
                self.assertTrue(all(run["status"] == "failed" for run in runs))
                errors = {run["tool_name"]: run["error"] for run in runs}
                self.assertEqual(errors["unknown.tool"], "unknown_tool")
                self.assertEqual(errors["note.add"], "missing_content")

    async def test_query_tools_return_results_and_record_success_runs(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            db_path = str(Path(temp_dir) / "query_tools.db")
            with XiaoAnMemoryStore(db_path) as memory_store:
                memory_store.insert_note(content="release report")
                memory_store.insert_work_activity(
                    app_name="VS Code",
                    window_title="Step 27 implementation",
                )
                memory_store.insert_summary(
                    summary_type="daily",
                    title="Daily report",
                    content="structured daily summary",
                    date="2026-06-22",
                )
                executor = ActionExecutor(
                    FakeRobotMotionSkill(),
                ...
                result = await executor.execute(OpenClawDecision(
                    ...
                    tool_calls=[
                        OpenClawToolCall(
                            name="note.search",
                            arguments={"keyword": "report"},
                        ),
                        OpenClawToolCall(
                            name="work_context.query",
                            arguments={"keyword": "Step 27"},
                        ),
                        OpenClawToolCall(
                            name="summary.query",
                ...
                self.assertEqual(
                    [action["name"] for action in result["executed_actions"]],
                    ["note.search", "work_context.query", "summary.query"],
                )
                self.assertEqual(result["skipped_actions"], [])
                runs = memory_store.query_recent_tool_runs(limit=10)
                self.assertEqual(
                    {run["tool_name"] for run in runs},
                    {"note.search", "work_context.query", "summary.query"},
                )
                self.assertTrue(all(run["status"] == "success" for run in runs))
