"""Manually test EmotionMonitorSkill sliding-window policy with EmotionDB."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Test Xiao An emotion policy with SQLite memory.")
    ...
    parser.add_argument("--repeat", type=int, default=1, help="Number of triggers to inject.")
...
async def main() -> None:
    ...
    db = EmotionDB(db_path=args.db)
    try:
        gateway = RobotGateway(url=args.url)
        skill = EmotionMonitorSkill(
            gateway=gateway,
            memory=db,
